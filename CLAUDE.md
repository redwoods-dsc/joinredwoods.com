...
**Every page has exactly one `h1`, and it's the page's title** — `ContentPage` renders `title` as one, and a page without a title opens on a `<PageTitle>`, which is one too. The logo is the nav's home link, not a heading. So body headings start at `##` and never skip a level on the way down. `base.css` sizes each level for that role — an `h2` is a section heading, not a second title — so a heading that looks too big or too small is a styling question, not a reason to pick a different level.

`ContentPage` frontmatter knobs, all optional:

| Key            | Default    | What it does                                                                                                                                                                                      |
| -------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `title`        | —          | Renders the intro header, with the title as the page's `h1`. Omit it and the page opens straight into body copy, which then needs a `<PageTitle>`.                                                |
...
Reach for `.astro` only when the page needs something Markdown can't express — computed frontmatter, `getStaticPaths()`, or bespoke layout. Needing a component is _not_ one of those reasons: `.mdx` imports components fine, and `src/pages/index.mdx` uses `<Button />`, `<PageTitle />` and `<Quote />` that way. If a `.md` page later needs one component, rename it to `.mdx` rather than rewriting it as `.astro`.

Field Notes articles are different: they live in the `field-notes` content collection (schema in `src/content.config.ts`) and are rendered by `src/pages/field-notes/[slug].astro`. Each one is a directory — `src/content/field-notes/<slug>/index.mdx`, with its images in an `images/` folder beside it and referenced relatively (`./images/foo.jpg`) from the body, from an `import`, and from the `image` frontmatter field. `src/assets/` is for artwork the site shares, not for one article's photographs. Don't add standalone pages to that collection, and don't spin up a new collection for a one-off page.
...
- **Articles carry their own metadata** from the collection schema, handed to `ContentPage` by `src/pages/field-notes/[slug].astro`: `description`, `subtitle`, `image`, `imageAlt`, `date`, and optional `modifiedDate`. A field the schema doesn't name is dropped by `z.object()` without a word, so adding a knob means touching `src/content.config.ts` and the route, not just the frontmatter. `Seo` turns those into `og:type=article`, `article:published_time`/`modified_time`, per-author `article:author` tags, and a `BlogPosting` block with `Person` authors and Redwoods as publisher. Non-article pages get an `Organization` block instead.
...
- **The default card is `src/assets/og-card.png`** — wordmark, tagline and trees on a _transparent_ canvas. The colour behind them is flattened in from `tokens.css` at build time rather than baked into the file, the same move `header-background.ts` makes, so re-theming can't leave a stale orange in the image everyone sees when a link is shared. If you re-cut the card, keep it transparent.
- **An article's `tone` grounds its card.** `ogImage()` takes the tone alongside the image and flattens the default card onto that tone's token — green, blue or orange, the same three `Callout` and `EntryCard` use — so a shared link unfurls in the colour the article already wears. Pages without a tone keep the accent. The wordmark is baked cream, so a new tone's ground has to clear 3:1 against it; green and blue are 3.99:1, which is AA Large and nothing more. An article supplying its own `image` is unaffected — an opaque photo covers the canvas.
...
- `scripts/` holds tooling that is **run by a person, never by the build**. `sync-members.mjs` is the only one: it reads the Redwoods Slack workspace and writes `src/data/members.json` plus `src/assets/members/`, and its output is committed. The build must not call Slack — the site is static, and a build that depends on a third-party API and a secret is one that fails for reasons nobody can see in the repo. How to run it, set up its token, and recover when it refuses to write is in "Updating the member list" in the README. What matters when you're editing the code:
  ...
  - **`.github/workflows/sync-members.yml` runs the same script** weekly and on demand, and pushes a `sync-members` branch, leaving the pull request for a person to open from the link in the run summary — Actions can't create one unless the repository allows it to, and it shouldn't need to. It skips `pnpm install`, because the script imports nothing outside `node:` — give the script a dependency and that workflow needs an install step too.
  - **Rangers are kept by hand in `src/data/rangers.ts`, keyed by slug rather than name**, because Slack names don't always read as you'd type them (Robin is `robin-di-capua`). A renamed Ranger gets a new slug, so the entry has to follow. A slug that matches no listed member fails silently — which is also what a Ranger who hasn't opted in looks like — so check the page after editing it.
